...
static Byte saltBuff[] = {50, 52, 48, 50, 49, 50, 48, 49};

static Byte ivBuff[]   = {-79, -109, 86, 19, 59, -24, 18, -60, -97, 52, -110, 9, -83, -79, 40, 42};
...
+ (NSData *)AESKeyForPassword:(NSString *)password{                  //Derive a key from a text password/passphrase
    ...
    NSData *salt = [NSData dataWithBytes:saltBuff length:kCCKeySizeDES];
...
+ (NSString *)AES256EncryptWithPlainText:(NSString *)plain {
    ...
    CCCryptorStatus cryptStatus = CCCrypt(kCCEncrypt, kCCAlgorithmAES128,kCCOptionPKCS7Padding,
                                          [[NSData AESKeyForPassword:@"wzsdxcghzcllzlbs"] bytes], kCCKeySizeAES256,
...
+ (NSString *)AES256DecryptWithCiphertext:(NSString *)ciphertexts{
    NSString *str = [ciphertexts stringByReplacingOccurrencesOfString:@"%3d" withString:@"="];
    NSString *str1 = [str stringByReplacingOccurrencesOfString:@"%2f" withString:@"/"];
    NSString *str2 = [str1 stringByReplacingOccurrencesOfString:@"%2b" withString:@"+"];

    NSData *cipherData = [NSData dataWithBase64EncodedString:str2];
    // 'key' should be 32 bytes for AES256, will be null-padded otherwise
    char keyPtr[kCCKeySizeAES256+1]; // room for terminator (unused)
    bzero(keyPtr, sizeof(keyPtr)); // fill with zeroes (for padding)
