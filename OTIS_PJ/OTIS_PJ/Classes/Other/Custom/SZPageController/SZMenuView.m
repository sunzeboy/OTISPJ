...
#import "SZMenuViewBtn.h"

@interface SZMenuView ()<UIScrollViewDelegate>

@property (nonatomic,strong)UIScrollView *MenuScrollView;
@property (nonatomic,strong)SZMenuViewBtn *selectedBtn;
@property (nonatomic,strong)SZFloodView  *line;
...
- (void)loadWithScollviewAndBtnWithTitles:(NSArray *)titles {
    ...
    for (int i = 0; i < titles.count; i++) {
        SZMenuViewBtn *btn = [[SZMenuViewBtn alloc ]initWithTitles:titles AndIndex:i];
        btn.tag = i;
...
- (void)layoutSubviews {
    ...
    SZMenuViewBtn *btn = nil;
    SZMenuViewBtn *btn1 = nil;
    self.sumWidth = 0;
...
- (void)drawRect:(CGRect)rect {
    ...
    if (self.style == MenuViewStyleDefault) {
        SZMenuViewBtn *btn = [self.MenuScrollView.subviews firstObject];
...
- (void)SelectedBtnMoveToCenterWithIndex:(int)index WithRate:(CGFloat)Pagerate {
    ...
    self.selectedBtn.selected = NO;
    SZMenuViewBtn *currentbtn = self.MenuScrollView.subviews[index];
    SZMenuViewBtn *nextBtn = self.MenuScrollView.subviews[index + 1];
...
- (void)MoveCodeWithIndex:(int )index {
    SZMenuViewBtn *btn = self.MenuScrollView.subviews[index];
...
- (void)selectWithIndex:(int)index AndOtherIndex:(int)tag {
    ...
    SZMenuViewBtn *otherbtn = self.MenuScrollView.subviews[tag];
